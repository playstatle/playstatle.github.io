...
//รับข้อความจากผู้ใช้
$id = $arrayJson['events'][0]['source']['userId'];//'Ub95654be5cf4144f8f16b50aeb2b2c73';
// $Name = $arrayJson['events'][0]['source']['userId'];

$message = $arrayJson['events'][0]['message']['text'];//'JQ40001A';
...
//        CreateLogContent($log_name, $id, $message, "System error, please try later.");
    ...
    if ($count_result > 0) {
        ...
        $found = true;
        CreateLogDB($conn,$id,$query_result['user_type'],$message,$result_get);
        replyMsg($arrayHeader, $arrayPostData);
...
function CreateLogDB($conn,$user_id,$user_type,$input,$output){
    $query = "INSERT INTO line_access_log(user_id,user_type,input,output) VALUE ('$user_id','$user_type','$input','$output')";
