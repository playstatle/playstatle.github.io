...
if(isset($name) && isset($email) && isset($pass) && !empty($id)){
	...
	$sql = "select name, lastname, userId from whatsapp_login where userId='{$id}' group by userId"; 
	$result = mysqli_query($connect,$sql) or die ("error".mysqli_error()); 
	$count_row = mysqli_num_rows($result);
	if($count_row < 1 && !empty($user_type)){ 
		
		$query = "INSERT INTO whatsapp_login(name,lastname,email,user_type,status,userId,pass,modified_date) VALUE ('$name','$lastname','$email','$user_type','INACTIVE','$id','$pass',NOW())"; 
